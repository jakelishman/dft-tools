...
main (int argc, char **argv)
{
    ...
    const double start_ratio = strtod_checked (argv[2]);
    const double inc         = strtod_checked (argv[3]);
    const double end_ratio   = strtod_checked (argv[4]);

    const int max = 1 + (0.5 * inc + end_ratio - start_ratio) / inc;

    for (int i = 0; i < max; i++)
        printf ("%.10g %.10g\n", a, (double) i * inc + start_ratio);

    return 0;
}

#define USAGE_STRING \
    "Usage: vary-ratio a start_ratio inc end_ratio\n\n"\
    "PARAMETERS\n"\
    "  a\n"\
    "    The base lattice parameter of the CsCl FeRh.\n"\
    "  start_ratio\n"\
    "    The start value of the c/a ratio to use.  Will probably be\n"\
    "    somewhat close to 1.\n"\
    "  inc\n"\
    "    The increment to apply to the start value of the ratio at each step.\n"\
    "  end_ratio\n"\
    "    The largest value of ratio to use.\n"
